    ...
    getAll: function() {
        return new Promise((resolve, reject) => {
            db.query(`SELECT * FROM ${TABLE_NAME}`, [], (err, result) => {
                if(err) { return reject({code: "sql_err", reason: `SQL_ERROR ${err.errno}`}); }
    ...
    getByUsername: function(username) {
        return new Promise((resolve, reject) => {
            db.query(`SELECT * FROM ${TABLE_NAME} WHERE username=?`, [username], (err, result) => {
                if(err) { return reject({code: "sql_err", reason: `SQL_ERROR ${err.errno}`}); }
                if(result.length < 1) { return reject({code: "invalid_credentials", reason: "No user with provided credentials"}); }
