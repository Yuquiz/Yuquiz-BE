import model from "../models/RoomQuizzes.js";

const FILLABLES = ["room_id", "quiz_id"];

export default {
    index: async function(req, res, next) {
        await model.getAll()
            .then(result => res.send({
                msg: "RoomQuiz fetch success",
                data: result
            }))
            .catch(err => next(err));
    },
    ...
    store: async function(req, res, next) {
        ...
        await model.store(data)
            .then(result => res.send({ msg: `RoomQuiz created with id:${result}` }))
            .catch(err => next(err));
    },
